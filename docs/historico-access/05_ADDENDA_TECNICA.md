...
| **N-18** | **El turno se descarta en cinco tablas** (`H01`, `E05`, `M_Poda`, `M_nMuestra`, `R09`) mientras `R08` sí lo persiste, y **no es automático derivarlo del lote**: comparado contra `core.lote.turno_id`, `H01` coincide en 30.536/30.536 filas (100%), pero `E05` coincide en solo 3.788/3.889 (97,4%) — las 101 restantes son siempre el mismo conflicto, turno `T09` en el origen contra `T11` en el maestro vigente. Ese 2,6% de discrepancia sistemática, no aleatoria, es lo que impide escribir la derivación sin decidir cuál de las dos fuentes vale | Medio |
