...
DECLARE v_n integer;
BEGIN
    TRUNCATE core.op_riego_semanal RESTART IDENTITY CASCADE;

    INSERT INTO core.op_riego_semanal (modulo_id, anio_semana, agua_m3, lamina_mm,
    ...
    JOIN core.t_calendario ca ON ca.fecha = r.fecha
    GROUP BY r.modulo_id, ca.anio_semana;

    GET DIAGNOSTICS v_n = ROW_COUNT;
    RAISE NOTICE 'Riego: % filas (módulo x semana), % de ellas con reparto estimado (D-7)',
        v_n, (SELECT count(*) FROM core.op_riego_semanal WHERE estimado);
END;
$$;

COMMENT ON PROCEDURE core.sp_cargar_riego() IS
    'Carga core.op_riego_semanal desde stg.v_riego_diario, sumando agua_m3 y lamina_mm por '
