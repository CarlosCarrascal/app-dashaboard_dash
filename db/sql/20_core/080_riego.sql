...
--
-- Grano: módulo × semana ISO (core.t_calendario.anio_semana), no módulo × campaña ×
...
CREATE TABLE IF NOT EXISTS core.op_riego_semanal (
    ...
    -- descarta la fila: la marca, igual que `sospechoso` en core.ev_baya_medicion.
    estimado         boolean NOT NULL DEFAULT false,
    UNIQUE (modulo_id, anio_semana)
);

COMMENT ON TABLE core.op_riego_semanal IS
