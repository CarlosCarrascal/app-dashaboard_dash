...
# Mismo valor por omisión que `dashboard/app.py` (hallado por barrido, ver
...
def _cargar_panel_inicial(_boot: int):
    ...
    """
    contenido = _leer_si_existe(config.XLSX_REPO)
    if contenido is None:
        return None, {"nombre": None, "error": f"No se encontró {config.XLSX_REPO}"}

    poda_contenido = _leer_si_existe(config.PODA_REPO)
    floracion_contenido = _leer_si_existe(config.FLORACION_REPO)
    panel = nucleo.cargar_panel(
        contenido, LAGS_POR_DEFECTO, poda_contenido, floracion_contenido
    )
    info = {
        "nombre": config.XLSX_REPO.name,
