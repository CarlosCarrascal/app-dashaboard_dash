...
def _precargar_lecturas(panel, sem):
    ...
    precargar(
        panel,
        {
            ...
            "fp:floracion:Frutos": lambda: nucleo.clima.rezago_floracion(
                panel.tabla, objetivo="Frutos"
            ),
    ...
    return ui.fila_kpi(
        [
            ...
            ui.kpi(
                ...
                nota=(
                    "Semanas del peak; la mediana es "
                    f"S{int(tray['Semana peak frutos'].median())}."
                ),
...
def _respuesta_corta(descomp) -> html.Div:
    ...
    respuesta = (
        ...
        "calendario queda señal para **Frutos** en "
        f"{', '.join(quedan_frutos) or 'ninguna variable'} "
            ...
            html.Div(
                ...
                children=[
                    ...
                    ui.panel(
                        ...
                        ayuda=(
                            "Comparación de asociaciones crudas y controladas para "
                            "cuajado y tamaño."
                        ),
                    ),
                    ui.panel(
                        ...
                        ayuda=(
                            "Lectura de una trayectoria concreta, sin confundirla con "
                            "el promedio del fundo."
                        ),
                    ),
                    ui.panel(
                        ...
                        ui.plegable(
                            ...
                            dcc.Dropdown(
                                id="fp-picos-clima",
                                options=[
                                    {"label": etiqueta(c), "value": c} for c in VARIABLES_PRE_PEAK
                                ],
                                value="TempMin",
                                clearable=False,
                                persistence=True,
                    ...
                    ui.panel(
                        ...
                        ayuda=(
                            "Relación exploratoria entre exposición pre-peak y cambio "
                            "observado de peso."
                        ),
                    ),
                    ui.panel(
...
def _render_desfases_shell(panel):
    ...
    return (
        _desfases_shell(sem, panel.tabla, resumen),
        objetivos,
        (objetivos[0] if objetivos else None),
    )
...
@callback(
    ...
    Input(PANEL_STORE, "data"),
    Input("fp-modulo", "value", allow_optional=True),
)
def _render_trayectoria(panel, modulo):
    ...
    serie = (
        panel.tabla[panel.tabla.celda == modulo]
        .dropna(subset=["Frutos", "Peso"])
        .sort_values("nsem")
    )
            ...
            [
                ui.kpi(
                    ...
                    nota=(
                        ...
                        f"{fila['Posición del peak'].lower()} de la ventana observada."
                    ),
                ),
                ui.kpi(
                    "Frutos en el peak",
                    f"{fila['Peak frutos/planta']:.1f}",
                ...
                ui.kpi(
                    "Cambios de sentido",
                    str(int(fila["Cambios de sentido"])),
    ...
    bloques.append(
        dcc.Tabs(
            children=[
                dcc.Tab(
                    label="Frutos + peso",
                    children=dcc.Graph(figure=fig_juntas, config={"displaylogo": False}),
                ),
                dcc.Tab(
                    label="Peak de frutos",
                    children=dcc.Graph(figure=fig_frutos, config={"displaylogo": False}),
                ),
                dcc.Tab(
                    label="Curva de peso",
                    children=dcc.Graph(figure=fig_peso, config={"displaylogo": False}),
                ),
            ]
        )
    )
    ...
    for objetivo, titulo in [("Frutos", "Frutos por planta"), ("Peso", "Peso del fruto")]:
        ...
        bloques.append(
            ...
            if sobreviven
                ...
                "info",
                f"Ninguna variable sobrevive al control del calendario para {titulo.lower()}.",
            )
        )
        if not sub.empty:
            cruda = sub.loc[sub["r sin controlar"].abs().idxmax()]
        ...
        poda_sub = (
            sub.dropna(subset=["r control poda"]) if "r control poda" in sub else sub.iloc[0:0]
        )
                ...
                ui.parrafo(
                    ...
                    + (
                        ...
                        if sobreviv_poda
                            ...
                            "ninguna variable mantiene p < 0,05 después de usar días "
                            "desde poda como control proxy."
                        )
                    )
                )
            )
    ...
    if "flores_promedio" not in panel.tabla.columns or panel.tabla.flores_promedio.isna().all():
        return html.Div(
            [
                ...
                ui.semaforo(
                    "info",
                    "No se cargó «DAtos mes.xlsx» (hoja EvFlores) — sin esto no hay "
                    "conteo real de flores para comparar contra Frutos.",
                ),
            ]
        )
    return html.Div(
...
def _render_floracion(panel, objetivo_flor):
    ...
    if rezago.empty:
        return ui.semaforo(
            "info",
            f"No hay suficiente solapamiento entre floración y {objetivo_flor} para esta prueba.",
        )
    ...
    fig.add_trace(
        go.Scatter(
            x=rezago.Rezago,
            y=rezago["r control módulo"],
            mode="lines+markers",
            name="Controlando módulo",
    ...
    fig.add_trace(
        go.Scatter(
            x=rezago.Rezago,
            y=rezago["r control módulo y calendario"],
            mode="lines+markers",
            name="Controlando módulo y calendario",
            line={"color": AZUL, "width": 2.8},
        )
    )
    ...
    fig.update_layout(
        height=380,
        xaxis_title=f"semanas de floración antes de {nombre_obj}",
        yaxis_title=f"correlación con {nombre_obj}",
        yaxis_range=[-1, 1],
        margin={"l": 10, "r": 10, "t": 10, "b": 10},
    ...
    r_mismo = (
        float(rezago.loc[rezago.Rezago == 0, "r control módulo y calendario"].iloc[0])
        if con_mismo
        else None
    )
    ...
    if con_mismo:
        ...
        mensaje = (
            ...
            f"celdas de {int(mejor['módulos'])} módulos) con "
            f"{int(mejor.Rezago)} semanas de anticipación."
        )
...
def _desfases_shell(sem, tabla, resumen) -> html.Div:
    if resumen.empty:
        return html.Div(
            [
                ui.titulo_seccion(
                    "Qué desfase explica mejor kg/ha, Frutos, Peso y Floración", "h4"
                ),
                ui.semaforo(
                    "info", "No hay suficientes semanas con Frutos y Peso para esta búsqueda."
                ),
            ]
        )
...
@callback(
    Output("fp-desfase-variable", "options"),
    Output("fp-desfase-variable", "value"),
    Input(PANEL_STORE, "data"),
...
def _render_desfase(panel, objetivo_sel, variable_sel):
    ...
    fig.update_layout(
        ...
        yaxis_title=f"correlación con {objetivo_sel}",
        yaxis_range=[-1, 1],
        margin={"l": 10, "r": 10, "t": 10, "b": 10},
        legend={"orientation": "h", "y": 1.14},
    )
...
def _picos_narrativa(panel) -> html.Div:
    ...
    if tray.empty or resumen.empty:
        return ui.semaforo(
            "info", "No hay suficientes pares Frutos–Peso para comparar el momento del peak."
        )

    fig = px.scatter(
        tray.sort_values("Semana peak frutos"),
        x="Semana peak frutos",
        y="Módulo",
        size="Peak frutos/planta",
        color="Días desde poda peak",
    ...
    posicion = (
        tray["Posición del peak"].value_counts().reindex(["Inicio", "Medio", "Final"], fill_value=0)
    )
    return html.Div(
        className="space-y-3",
        children=[
...
def _picos_shell(panel) -> html.Div:
    ...
    if tray.empty or resumen.empty:
        return html.Div(
            [
                ui.titulo_seccion(
                    "Por qué el peak de frutos aparece al inicio, medio o final", "h4"
                ),
                ui.semaforo(
                    "info",
                    "No hay suficientes pares Frutos–Peso por módulo para comparar la "
                    "posición del peak.",
                ),
            ]
        )
    ...
    columna_peak_resumen = next(
        (
            ...
            if str(c).lower().startswith("posici") and "peak" in str(c).lower()
        ),
        None,
    )
...
@callback(
    ...
    Input(PANEL_STORE, "data"),
    Input("fp-picos-clima", "value", allow_optional=True),
)
def _render_picos(panel, variable):
    ...
    x = (
        "Días desde poda peak"
        if tray["Días desde poda peak"].notna().any()
        else "Semana peak frutos"
    )
    fig = px.scatter(
        tray,
        x=x,
        y=columna,
        color="Posición del peak",
        hover_name="Módulo",
...
@callback(
    ...
    Input(PANEL_STORE, "data"),
    Input("fp-peso-clima", "value", allow_optional=True),
)
