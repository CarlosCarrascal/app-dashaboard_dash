...
from analitica.config import FEATURES, etiqueta
...
registrar_pagina_legacy(
    __name__,
    path="/modelo/explicacion",
    name="Legacy · SHAP 2025",
    order=2,
...
def _disponibles(panel) -> list[str]:
    return [
        objetivo
        for objetivo in OBJETIVOS_SHAP
...
@callback(
    Output("celda-semana", "options"),
    Output("celda-semana", "value"),
    Input(PANEL_STORE, "data"),
    Input("celda-modulo", "value"),
)
...
@callback(
    ...
    Input(PANEL_STORE, "data"),
    Input("explicacion-objetivo", "value"),
)
        ...
        dcc.Graph(
            figure=_estilo_figura(
                g.dependencia_shap(
                    ...
                    ajuste.X,
                    var,
                    panel.tabla.loc[ajuste.X.index, "nsem"],
                    unidad,
                ),
                380,
            ),
    ...
    return ui.fila_kpi(
        [
            ...
            ui.kpi(
                ...
                ui.miles(prediccion),
                nota=(
                    "Diferencia frente a observado: "
                    f"{ui.miles(prediccion - fila.KgHa, '{:+,.0f}') }."
                ),
            ),
            ui.kpi(
                "Riego de esa semana",
    ...
    return html.Div(
        ...
        children=[
            ...
            ui.panel(
                ...
                ayuda=(
                    "Prueba interna de consistencia de SHAP, separada de la "
            ...
            ui.panel(
                ...
                ui.tabla_desde_df(
                    detalle, formato={"Valor": "{:.2f}", "Efecto (kg/ha)": "{:+.0f}"}
                ),
