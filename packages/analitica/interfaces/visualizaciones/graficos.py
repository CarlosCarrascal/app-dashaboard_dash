...
from analitica.config import AZUL, ETIQUETAS, GRIS, NARANJA, ROJO, VERDE, etiqueta


class _ModuloDiferido:
    """Carga Plotly solo cuando una figura realmente se solicita."""
    ...
    def __getattr__(self, atributo: str):
        ...
        return getattr(self._modulo, atributo)


px = _ModuloDiferido("plotly.express")
go = _ModuloDiferido("plotly.graph_objects")
...
def matriz_correlacion(corr: pd.DataFrame) -> go.Figure:
    fig = px.imshow(
        ...
        x=list(corr.columns),
        y=list(corr.index),
        color_continuous_scale=DIVERGENTE,
        zmin=-1,
        zmax=1,
        text_auto=".2f",
        aspect="auto",
...
def dispersion_contra_objetivo(tabla: pd.DataFrame, col: str) -> go.Figure:
    ...
    fig = px.scatter(
        ...
        y="KgHa",
        color="nsem",
        hover_data=["Fundo", "Modulo", "Semana"],
    ...
    fig.add_trace(
        go.Scatter(
            x=xs,
            y=pendiente * xs + origen,
            mode="lines",
            line={"color": GRIS, "dash": "dash"},
            name=f"ajuste lineal · r={r:+.2f}",
...
def dependencia_shap(
    ...
    fig = px.scatter(
        ...
        labels={
            "x": etiqueta(col),
            "y": f"efecto SHAP de {etiqueta(col)} ({unidad})",
            "color": "semana",
        },
    )
...
def waterfall(base: float, contribuciones: pd.Series, prediccion: float, titulo: str) -> go.Figure:
...
def _doble_eje(
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x, y=y1, mode="lines+markers", name=nombre1, line={"color": AZUL}))
    fig.add_trace(
        go.Scatter(
            x=x, y=y2, mode="lines", name=nombre2, yaxis="y2", line={"color": color2, "dash": "dot"}
        )
    )
    return _limpio(
        fig,
...
def mapa_por_modulo(porm: pd.DataFrame, columnas: list[str]) -> go.Figure:
    fig = px.imshow(
        porm[columnas].values,
        x=columnas,
        y=porm["Módulo"].tolist(),
        color_continuous_scale=DIVERGENTE,
        zmin=-1,
        zmax=1,
        text_auto=".2f",
...
def ventana_de_cosecha(porm: pd.DataFrame, clave: str = "TempMin") -> go.Figure:
    ...
    fig = px.scatter(
        ...
        range_color=[-1, 1],
        hover_name="Módulo",
        hover_data=["Pico", "kg/ha medio"],
        labels={"Inicio": "primera semana con cosecha", col: "correlación Temp. mín ↔ kg/ha"},
    )
    fig.add_hline(y=0, line_dash="dash", line_color="#888")
    return _limpio(fig, 440)
...
def reparto_varianza(pct_entre: float, pct_dentro: float) -> go.Figure:
    fig = go.Figure(
        go.Bar(
            x=[pct_entre, pct_dentro],
            y=["variación del kg/ha"] * 2,
            orientation="h",
            text=[f"entre semanas · {pct_entre:.0f}%", f"entre módulos · {pct_dentro:.0f}%"],
            textposition="inside",
            marker={"color": [AZUL, NARANJA]},
            hovertemplate="%{text}<extra></extra>",
        )
    )
    return _limpio(
        fig, 170, barmode="stack", showlegend=False, xaxis_title="% de la varianza total del kg/ha"
    )
