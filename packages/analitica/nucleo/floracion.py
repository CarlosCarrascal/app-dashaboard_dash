...
def _agregar_turno_a_modulo(ev: pd.DataFrame) -> pd.DataFrame:
    ...
    agregado["flores_dispersion_relativa"] = agregado["flores_desvio"] / agregado[
        "flores_promedio"
    ].replace(0, np.nan)
    return agregado
...
def integrar_floracion(
    ...
    if not contenido:
        hallazgos.append(
            _hallazgo(
                "floracion_no_cargada",
                "No se cargó EvFlores (DAtos mes.xlsx)",
                "media",
    ...
    try:
        ...
        hallazgos.append(
            _hallazgo(
                "floracion_formato",
                "No se pudo leer la hoja EvFlores",
                "media",
    ...
    if crudo.shape[1] < len(_COLUMNAS):
        hallazgos.append(
            _hallazgo(
                "floracion_formato",
                "EvFlores no tiene el formato esperado",
                "media",
                f"La hoja trae {crudo.shape[1]} columnas y se esperaban al menos {len(_COLUMNAS)}.",
    ...
    ev["Modulo"] = ev["Modulo"].astype(str).str.strip().replace({"M10A": "M10", "M10B": "M10"})
    ...
    if ev.empty:
        disponibles = sorted(
            pd.to_numeric(crudo.iloc[:, 3], errors="coerce").dropna().astype(int).unique().tolist()
        )
        hallazgos.append(
            _hallazgo(
                "floracion_campania",
                "EvFlores no contiene el año del panel",
                "media",
    ...
    if sin_alias:
        hallazgos.append(
            _hallazgo(
                "floracion_alias_fundo",
                "Fundo sin equivalencia para EvFlores",
                "media",
    ...
    hallazgos.append(
        _hallazgo(
            "floracion_integrada",
            "Floración real integrada al panel",
            "baja",
    ...
    if not dispersos.empty:
        hallazgos.append(
            _hallazgo(
                "floracion_dispersa_modulo",
                "La floración no es homogénea entre turnos",
                "media",
