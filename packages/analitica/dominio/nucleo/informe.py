...
from analitica.config import FEATURES, GLOSARIO, etiqueta

from . import clima as cl
from .contratos import Panel
...
NOMBRES_PANEL = {
    ...
    "riego_lt_planta": "Riego (L/planta)",
    ...
    "ETo": "ETo (mm)",
    "DPV": "DPV (kPa)",
    **{f: etiqueta(f) for f in COLUMNAS_LAG},
...
def _metodologia(panel: Panel) -> pd.DataFrame:
    filas = [
        (
            "Grano del panel",
            "Una fila por Fundo × Módulo × Semana. No hay lógica de "
            "turnos: el riego se consolida a módulo al leer el archivo.",
        ),
        (
            "Variable objetivo",
            "kg/ha = kilos ÷ área cosechada, ponderado por área cuando "
            "hay varias filas para la misma celda.",
        ),
        (
            "Riego",
            "Se usa «L/planta», que es la columna agregada al grano del módulo. "
        ...
        (
            "Correlaciones del clima",
            "Se calculan sobre el panel agregado a una fila por "
            "semana, que es el grano al que el dato existe.",
        ),
        (
            "Control del calendario",
            f"Polinomio de grado {cl.GRADO_TENDENCIA} sobre el "
            "número de semana. Un control lineal no describe la "
            "joroba de la campaña y deja pasar la confusión.",
        ),
        (
            "Rezagos",
            f"Se prueban de 0 a {max(cl.LAGS)} semanas, siempre con y sin "
            "tendencia. Solo la versión sin tendencia es interpretable.",
        ),
        (
            "Placebo",
            "Se contrastan las variables reales contra series inventadas con la "
            "misma forma estacional, para medir cuánto de la correlación es forma.",
        ),
        (
            "Tres capas",
            "Asociación estadística, aporte predictivo y efecto agronómico "
            "estimado se reportan por separado. Esta campaña cubre las dos "
            "primeras; no identifica todavía la tercera.",
        ),
        (
            "SHAP",
            "Explica cómo XGBoost reparte una predicción. No se interpreta como "
            "efecto causal ni como recomendación de intervención.",
        ),
        (
            "Fases agronómicas",
            "La poda y los días desde poda ya están incorporados como "
            "reloj biológico proxy. La fase fenológica observada todavía "
            "no está disponible.",
        ),
        (
            "Frutos y peso",
            "Se analizan como resultados biológicos secundarios. No entran "
            "como predictores de kg/ha porque son sus componentes.",
        ),
        (
            "DML",
            "No está implementado: antes deben definirse tratamiento, confusores, "
...
def construir(
    ...
    hojas = [
        ...
        Hoja(
            "Resumen semanal",
            "El fundo agregado a una fila por semana",
    ...
    if "clima" in incluir:
        ...
        hojas.insert(
            ...
            Hoja(
                "Correlaciones",
                "Correlación de cada variable con el kg/ha",
        ...
        hojas.insert(
            ...
            Hoja(
                "Control del calendario",
                "¿Sobrevive la correlación al descontar la estación?",
        ...
        hojas.insert(
            ...
            Hoja(
                "Rezagos",
                "Correlación desplazando el clima k semanas",
        ...
        hojas.insert(
            ...
            Hoja(
                "Placebo",
                "Series sin significado físico, contra las reales",
        ...
        if sem.Frutos.notna().sum() >= 10:
            hojas.insert(
                ...
                Hoja(
                    "Frutos y peso",
                    "El clima/riego, contra los dos componentes de kg/ha",
            ...
            hojas.insert(
                7,
                Hoja(
                    "Trayectoria biologica",
                    "Peak de frutos y cambio neto del peso por módulo",
    ...
    if "modulo" in incluir:
        ...
        hojas.insert(
            ...
            Hoja(
                "Por modulo",
                "La correlación calculada dentro de cada módulo",
        ...
        hojas.insert(
            ...
            Hoja(
                "Signo vs ventana",
                "¿El signo lo decide cuándo cosecha el módulo?",
    ...
    meta = [
        ...
        ("Capacidad predictiva", "El tablero no pronostica. Ver la hoja «Limitaciones»."),
        (
